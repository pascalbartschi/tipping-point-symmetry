...
fig_1a <- ggplot() + theme(panel.background = element_blank()) + labs(x = "Environmental driver", y = "Concentration")
fig_1b <- ggplot() + theme(panel.background = element_blank()) + labs(x = "Environmental driver")
fig_1c <- plot_trajectory_symmetry(res = bush_res, trajectory = "recovery", typ = "substrate") # + 
  # labs(y = paste(expression(Log[10](Concentration)), "[μM]"))
fig_1d <- plot_trajectory_symmetry(res = bush_res, trajectory = "collapse", typ = "substrate") + labs(y = "")
...
fig3b <- ggplot(data = df3 %>% filter(panel == "substrates"), 
                mapping = aes(x = time, y= concentration, color = species_plot)) +
        geom_step()  +
        labs(x  = "Time", y = "Concentration") + # [μM]
        scale_color_manual(values = c("#e85050", "#5da1df")) +
        theme(axis.title.x = element_blank())
